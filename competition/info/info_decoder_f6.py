...
#
# ρ 和"错几个比特"的换算（干净 ±1 符号，共 64 个）：ρ = 1 - k/32，k 是错误比特数
#     ρ=0.875 → 错 4 个      ρ=0.75 → 错 8 个      ρ=0.60 → 错 13 个
# 硬路径自己的容错上限才 ACCESS_MAX_HAMMING_LOOSE=4，所以 0.60 相当于比硬路径
# 松了 3 倍，现场日志里出现过 "softAC=40 但只有 3 个 Header 过、汉明距离直方图
# AC0..4 只统计到 2 个"——剩下 38 次全是噪声里的假同步。0.75 让软路径仍比硬路径
# 宽松（8 > 4，软同步的意义就在这），但不至于把噪声当包头。
#
# 调法：
#   日志里 softAC 一直是 0    → 往下调到 0.65
#   softAC 很多但 Header 全废 → 往上调到 0.80~0.875
# 别低于 0.6：64 个符号的纯噪声相关标准差约 0.125，再低就淹在误报里了。
SOFT_CORR_MIN = 0.75
