import socket
import struct
import time
import datetime
import xmlrpc.client
import rclpy
from rclpy.node import Node
from std_msgs.msg import String, Int32
import subprocess
import os
import signal

# ================= 核心配置参数 =================
UDP_IP = "127.0.0.1"
UDP_PORT = 14348
RPC_URL = "http://127.0.0.1:8080" 

# 【极其重要】：请务必将这里替换为你电脑上 GNU Radio 生成的 Python 脚本的绝对路径！
# 例如: "/home/auto/桌面/SDR-Zero/SDR-Zero/tx_radio_s.py"
GRC_SCRIPT_PATH = "tx_radio.py"

# 黑匣子录制配置
RECORD_STREAM = True
RECORD_PREFIX = "jamming_record"

# 干扰波密码配置
ACCESS_CODE_HEX = "16E8D377151C712D"
AC_NORMAL = bin(int(ACCESS_CODE_HEX, 16))[2:].zfill(64)
AC_INVERTED = "".join(['1' if b == '0' else '0' for b in AC_NORMAL])
FUZZY_NORMAL = AC_NORMAL[16:]
FUZZY_INVERTED = AC_INVERTED[16:]

EXPECTED_HEADER_BE = bytes([0x00, 0x0F, 0x00, 0x0F])
EXPECTED_HEADER_LE = bytes([0x0F, 0x00, 0x0F, 0x00])
AIR_FRAME_LEN = 27

# 官方干扰波频率对照表 (频率 Hz, Sensitivity)
FREQ_MAP = {
    'RED':  [(432200000, 2.8323), (432500000, 2.5809), (432800000, 0.6646)],
    'BLUE': [(434920000, 2.8323), (434620000, 2.5809), (434320000, 0.6646)]
}

# ================= 大疆 RM 官方 CRC 校验表 =================
CRC8_TAB =[
    
]
CRC16_TAB =[
    0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf, 0x8c48, 0x9dc1, 0xaf5a, 0xbed3, 0xca6c, 0xdbe5, 0xe97e, 0xf8f7,
    0x1081, 0x0108, 0x3393, 0x221a, 0x56a5, 0x472c, 0x75b7, 0x643e, 0x9cc9, 0x8d40, 0xbfdb, 0xae52, 0xdaed, 0xcb64, 0xf9ff, 0xe876,
    0x2102, 0x308b, 0x0210, 0x1399, 0x6726, 0x76af, 0x4434, 0x55bd, 0xad4a, 0xbcc3, 0x8e58, 0x9fd1, 0xeb6e, 0xfae7, 0xc87c, 0xd9f5,
    0x3183, 0x200a, 0x1291, 0x0318, 0x77a7, 0x662e, 0x54b5, 0x453c, 0xbdcb, 0xac42, 0x9ed9, 0x8f50, 0xfbef, 0xea66, 0xd8fd, 0xc974,
    0x4204, 0x538d, 0x6116, 0x709f, 0x0420, 0x15a9, 0x2732, 0x36bb, 0xce4c, 0xdfc5, 0xed5e, 0xfcd7, 0x8868, 0x99e1, 0xab7a, 0xbaf3,
    0x5285, 0x430c, 0x7197, 0x601e, 0x14a1, 0x0528, 0x37b3, 0x263a, 0xdecd, 0xcf44, 0xfddf, 0xec56, 0x98e9, 0x8960, 0xbbfb, 0xaa72,
    0x6306, 0x728f, 0x4014, 0x519d, 0x2522, 0x34ab, 0x0630, 0x17b9, 0xef4e, 0xfec7, 0xcc5c, 0xddd5, 0xa96a, 0xb8e3, 0x8a78, 0x9bf1,
    0x7387, 0x620e, 0x5095, 0x411c, 0x35a3, 0x242a, 0x16b1, 0x0738, 0xffcf, 0xee46, 0xdcdd, 0xcd54, 0xb9eb, 0xa862, 0x9af9, 0x8b70,
    0x8408, 0x9581, 0xa71a, 0xb693, 0xc22c, 0xd3a5, 0xe13e, 0xf0b7, 0x0840, 0x19c9, 0x2b52, 0x3adb, 0x4e64, 0x5fed, 0x6d76, 0x7cff,
    0x9489, 0x8500, 0xb79b, 0xa612, 0xd2ad, 0xc324, 0xf1bf, 0xe036, 0x18c1, 0x0948, 0x3bd3, 0x2a5a, 0x5ee5, 0x4f6c, 0x7df7, 0x6c7e,
    0xa50a, 0xb483, 0x8618, 0x9791, 0xe32e, 0xf2a7, 0xc03c, 0xd1b5, 0x2942, 0x38cb, 0x0a50, 0x1bd9, 0x6f66, 0x7eef, 0x4c74, 0x5dfd,
    0xb58b, 0xa402, 0x9699, 0x8710, 0xf3af, 0xe226, 0xd0bd, 0xc134, 0x39c3, 0x284a, 0x1ad1, 0x0b58, 0x7fe7, 0x6e6e, 0x5cf5, 0x4d7c,
    0xc60c, 0xd785, 0xe51e, 0xf497, 0x8028, 0x91a1, 0xa33a, 0xb2b3, 0x4a44, 0x5bcd, 0x6956, 0x78df, 0x0c60, 0x1de9, 0x2f72, 0x3efb,
    0xd68d, 0xc704, 0xf59f, 0xe416, 0x90a9, 0x8120, 0xb3bb, 0xa232, 0x5ac5, 0x4b4c, 0x79d7, 0x685e, 0x1ce1, 0x0d68, 0x3ff3, 0x2e7a,
    0xe70e, 0xf687, 0xc41c, 0xd595, 0xa12a, 0xb0a3, 0x8238, 0x93b1, 0x6b46, 0x7acf, 0x4854, 0x59dd, 0x2d62, 0x3ceb, 0x0e70, 0x1ff9,
    0xf78f, 0xe606, 0xd49d, 0xc514, 0xb1ab, 0xa022, 0x92b9, 0x8330, 0x7bc7, 0x6a4e, 0x58d5, 0x495c, 0x3de3, 0x2c6a, 0x1ef1, 0x0f78
]

def calc_crc8(data):
    crc = 0xFF
    for byte in data: crc = CRC8_TAB[crc ^ byte]
    return crc

def calc_crc16(data):
    crc = 0xFFFF
    for byte in data: crc = ((crc >> 8) ^ CRC16_TAB[(crc ^ byte) & 0xFF]) & 0xFFFF
    return crc

def bits_to_bytes(bit_string):
    try: return bytes(int(bit_string[i:i+8], 2) for i in range(0, len(bit_string), 8))
    except: return b""

def invert_bits(bit_string):
    return "".join(['1' if b == '0' else '0' for b in bit_string])

# ================= ROS2 节点 =================
class JammingControllerNode(Node):
    def __init__(self):
        super().__init__('jamming_controller_node')
        self.publisher_ = self.create_publisher(String, 'radar/jamming_key', 10)
        self.enemy_camp = 'BLUE'
        self.get_logger().info('干扰波解码已启动')

    def publish_key(self, key_str):
        msg = String()
        msg.data = key_str
        self.publisher_.publish(msg)
        self.get_logger().info(f'密钥截获: {key_str}')

def parse_0x0A06(data):
    if len(data) < 6: return None
    key_bytes = data[:6]
    clean_key = "".join([chr(b) for b in key_bytes if 32 <= b <= 126]).strip()
    return clean_key

# ================= 全局看门狗变量 =================
grc_process = None
grc_rpc = None

def restart_grc_process():
    """重启 GNU Radio 程序"""
    global grc_process, grc_rpc
    if grc_process is not None:
        print("[看门狗] 正在关闭硬件...")
        try:
            os.kill(grc_process.pid, signal.SIGTERM)
            grc_process.wait(timeout=2)
        except:
            try: os.kill(grc_process.pid, signal.SIGKILL)
            except: pass
    
    print("[看门狗] 正在拉起 GNU Radio 物理层...")
    grc_process = subprocess.Popen(["python3", GRC_SCRIPT_PATH])
    time.sleep(3.0)  # 等待 PlutoSDR 初始化
    
    try:
        grc_rpc = xmlrpc.client.ServerProxy(RPC_URL)
        print("[看门狗] 物理层 RPC 接口重连成功！")
    except Exception as e:
        print(f"[看门狗] RPC 接口重连失败: {e}")

def main():
    global grc_rpc
    rclpy.init()
    node = JammingControllerNode()

    # 1. 启动看门狗，首次拉起 GNU Radio
    restart_grc_process()

    # 2. 录制功能初始化
    now = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    current_record_file = f"{RECORD_PREFIX}_{now}.txt"
    file_recorder = None
    if RECORD_STREAM:
        try:
            file_recorder = open(current_record_file, 'w')
            node.get_logger().info(f"黑匣子已就绪: {current_record_file}")
        except Exception as e: pass

    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.setblocking(False)
    sock.bind((UDP_IP, UDP_PORT))

    bit_buffer = ""
    serial_buffer = bytearray()
    
    last_success_time = time.time()
    last_udp_recv_time = time.time() # 看门狗心跳
    scan_idx = 0
    locked = False
    last_key = ""

    try:
        while rclpy.ok():
            current_time = time.time()

            # 防卡死机制：超过 2 秒没收到任何 UDP 数据 (底噪)
            if current_time - last_udp_recv_time > 2.0:
                node.get_logger().error("硬件心跳丢失！检测到数据断流，执行紧急重启！")
                restart_grc_process()
                last_udp_recv_time = time.time()
                last_success_time = time.time()
                bit_buffer = ""; serial_buffer.clear()
                locked = False
                continue

            # --- 自动扫频机制 (失锁 3 秒触发) ---
            if not locked and (current_time - last_success_time > 3.0) and grc_rpc:
                config_list = FREQ_MAP[node.enemy_camp]
                scan_idx = (scan_idx + 1) % len(config_list)
                target_f, target_s = config_list[scan_idx]
                try:
                    grc_rpc.set_target_freq(target_f)
                    grc_rpc.set_target_sens(target_s)
                    node.get_logger().info(f"扫频中: {target_f/1e6} MHz (Sens: {target_s})")
                except Exception as e: pass
                last_success_time = current_time
                bit_buffer = ""; serial_buffer.clear()

            # --- 接收 UDP 喂狗与写录音 ---
            try:
                data, _ = sock.recvfrom(16384)
                last_udp_recv_time = time.time() 
                
                incoming_bits = ''.join(str(b) for b in data)
                bit_buffer += incoming_bits
                if file_recorder:
                    file_recorder.write(incoming_bits + "\n")
                    file_recorder.flush()
            except BlockingIOError: pass

            # --- 第一层：极性双搜与模糊切片 ---
            while len(bit_buffer) >= AIR_FRAME_LEN * 8:
                idx_normal = bit_buffer.find(FUZZY_NORMAL)
                idx_inv = bit_buffer.find(FUZZY_INVERTED)
                
                target_idx = -1
                is_inv = False
                
                if idx_normal != -1 and (idx_inv == -1 or idx_normal < idx_inv):
                    target_idx = idx_normal; is_inv = False
                elif idx_inv != -1:
                    target_idx = idx_inv; is_inv = True
                
                if target_idx == -1:
                    bit_buffer = bit_buffer[-63:]; break
                
                start_idx = target_idx - 16
                if start_idx < 0:
                    bit_buffer = bit_buffer[target_idx+1:]; continue
                
                if len(bit_buffer) < start_idx + (AIR_FRAME_LEN * 8): break

                raw_frame_bits = bit_buffer[start_idx : start_idx + (AIR_FRAME_LEN * 8)]
                # ⚡ 若发现反极性，自动翻转波形
                frame_bits = invert_bits(raw_frame_bits) if is_inv else raw_frame_bits
                frame_bytes = bits_to_bytes(frame_bits)
                
                # Header 校验 (宽松策略，让 CRC 决定生死)
                if frame_bytes: 
                    serial_buffer.extend(frame_bytes[12:27]) # 只切下 15 字节纯肉馅！
                
                bit_buffer = bit_buffer[start_idx + (AIR_FRAME_LEN * 8):]

            # --- 第二层：串口防弹装甲 ---
            while len(serial_buffer) >= 9:
                sof_idx = serial_buffer.find(0xA5)
                if sof_idx == -1: serial_buffer.clear(); break
                if sof_idx > 0: serial_buffer = serial_buffer[sof_idx:]
                if len(serial_buffer) < 5: break

                # CRC8 校验失败：只滑 1 字节！
                if calc_crc8(serial_buffer[:4]) != serial_buffer[4]:
                    serial_buffer = serial_buffer[1:]; continue

                data_length = struct.unpack('<H', serial_buffer[1:3])[0]
                expected_len = 5 + 2 + data_length + 2
                
                if expected_len > 128: 
                    serial_buffer = serial_buffer[1:]; continue
                if len(serial_buffer) < expected_len: break

                full_frame = serial_buffer[:expected_len]
                # CRC16 校验失败：只滑 1 字节！
                if calc_crc16(full_frame[:-2]) != struct.unpack('<H', full_frame[-2:])[0]:
                    serial_buffer = serial_buffer[1:]; continue
                
                # ⚡ 冲破所有阻碍，提取真理！
                last_success_time = time.time(); locked = True
                cmd_id = struct.unpack('<H', full_frame[5:7])[0]
                if cmd_id == 0x0A06:
                    key = parse_0x0A06(full_frame[7 : 7 + data_length])
                    # 去重发布逻辑
                    #if key and key != last_key:
                    node.publish_key(key)
                    last_key = key
                
                serial_buffer = serial_buffer[expected_len:]

            # 状态重置
            if locked and (time.time() - last_success_time > 5.0):
                locked = False; node.get_logger().warn("锁定失效，进入扫频")

            rclpy.spin_once(node, timeout_sec=0.001)

    except KeyboardInterrupt:
        pass
    finally:
        if file_recorder: file_recorder.close()
        if grc_process is not None:
            try: os.kill(grc_process.pid, signal.SIGKILL)
            except: pass
        node.destroy_node()
        if rclpy.ok(): rclpy.shutdown()

if __name__ == '__main__':
    main()