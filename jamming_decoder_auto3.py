import socket
import struct
import time
import xmlrpc.client
import rclpy
from rclpy.node import Node
from std_msgs.msg import String, Int32

# ================= 配置参数 =================
UDP_IP = "127.0.0.1"
UDP_PORT = 14348
RPC_URL = "http://127.0.0.1:8080" 

ACCESS_CODE_HEX = "16E8D377151C712D"
# 1. 核心改进：生成模糊同步码 (取后 48 位，即后 6 字节)
# 这样即使深大信号开头漏掉了几个 bit，也能通过后缀锁死信号
ACCESS_CODE_BITS = bin(int(ACCESS_CODE_HEX, 16))[2:].zfill(64)
FUZZY_SYNC_BITS = ACCESS_CODE_BITS[16:] # 抛弃前16位，用后48位做钩子

EXPECTED_HEADER = bytes([0x0F, 0x00, 0x0F, 0x00])
AIR_FRAME_LEN = 27

FREQ_MAP = {
    'RED':  [(432200000, 2.8323), (432500000, 2.5809), (432800000, 0.6646)],
    'BLUE': [(434920000, 2.8323), (434620000, 2.5809), (434320000, 0.6646)]
}

# ================= 校验表 =================
CRC8_TAB = []
CRC16_TAB = [0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf, 0x8c48, 0x9dc1, 0xaf5a, 0xbed3, 0xca6c, 0xdbe5, 0xe97e, 0xf8f7, 0x1081, 0x0108, 0x3393, 0x221a, 0x56a5, 0x472c, 0x75b7, 0x643e, 0x9cc9, 0x8d40, 0xbfdb, 0xae52, 0xdaed, 0xcb64, 0xf9ff, 0xe876, 0x2102, 0x308b, 0x0210, 0x1399, 0x6726, 0x76af, 0x4434, 0x55bd, 0xad4a, 0xbcc3, 0x8e58, 0x9fd1, 0xeb6e, 0xfae7, 0xc87c, 0xd9f5, 0x3183, 0x200a, 0x1291, 0x0318, 0x77a7, 0x662e, 0x54b5, 0x453c, 0xbdcb, 0xac42, 0x9ed9, 0x8f50, 0xfbef, 0xea66, 0xd8fd, 0xc974, 0x4204, 0x538d, 0x6116, 0x709f, 0x0420, 0x15a9, 0x2732, 0x36bb, 0xce4c, 0xdfc5, 0xed5e, 0xfcd7, 0x8868, 0x99e1, 0xab7a, 0xbaf3, 0x5285, 0x430c, 0x7197, 0x601e, 0x14a1, 0x0528, 0x37b3, 0x263a, 0xdecd, 0xcf44, 0xfddf, 0xec56, 0x98e9, 0x8960, 0xbbfb, 0xaa72, 0x6306, 0x728f, 0x4014, 0x519d, 0x2522, 0x34ab, 0x0630, 0x17b9, 0xef4e, 0xfec7, 0xcc5c, 0xddd5, 0xa96a, 0xb8e3, 0x8a78, 0x9bf1, 0x7387, 0x620e, 0x5095, 0x411c, 0x35a3, 0x242a, 0x16b1, 0x0738, 0xffcf, 0xee46, 0xdcdd, 0xcd54, 0xb9eb, 0xa862, 0x9af9, 0x8b70, 0x8408, 0x9581, 0xa71a, 0xb693, 0xc22c, 0xd3a5, 0xe13e, 0xf0b7, 0x0840, 0x19c9, 0x2b52, 0x3adb, 0x4e64, 0x5fed, 0x6d76, 0x7cff, 0x9489, 0x8500, 0xb79b, 0xa612, 0xd2ad, 0xc324, 0xf1bf, 0xe036, 0x18c1, 0x0948, 0x3bd3, 0x2a5a, 0x5ee5, 0x4f6c, 0x7df7, 0x6c7e, 0xa50a, 0xb483, 0x8618, 0x9791, 0xe32e, 0xf2a7, 0xc03c, 0xd1b5, 0x2942, 0x38cb, 0x0a50, 0x1bd9, 0x6f66, 0x7eef, 0x4c74, 0x5dfd, 0xb58b, 0xa402, 0x9699, 0x8710, 0xf3af, 0xe226, 0xd0bd, 0xc134, 0x39c3, 0x284a, 0x1ad1, 0x0b58, 0x7fe7, 0x6e6e, 0x5cf5, 0x4d7c, 0xc60c, 0xd785, 0xe51e, 0xf497, 0x8028, 0x91a1, 0xa33a, 0xb2b3, 0x4a44, 0x5bcd, 0x6956, 0x78df, 0x0c60, 0x1de9, 0x2f72, 0x3efb, 0xd68d, 0xc704, 0xf59f, 0xe416, 0x90a9, 0x8120, 0xb3bb, 0xa232, 0x5ac5, 0x4b4c, 0x79d7, 0x685e, 0x1ce1, 0x0d68, 0x3ff3, 0x2e7a, 0xe70e, 0xf687, 0xc41c, 0xd595, 0xa12a, 0xb0a3, 0x8238, 0x93b1, 0x6b46, 0x7acf, 0x4854, 0x59dd, 0x2d62, 0x3ceb, 0x0e70, 0x1ff9, 0xf78f, 0xe606, 0xd49d, 0xc514, 0xb1ab, 0xa022, 0x92b9, 0x8330, 0x7bc7, 0x6a4e, 0x58d5, 0x495c, 0x3de3, 0x2c6a, 0x1ef1, 0x0f78]

def calc_crc8(data):
    crc = 0xFF
    for byte in data: crc = CRC8_TAB[crc ^ byte]
    return crc

def calc_crc16(data):
    crc = 0xFFFF
    for byte in data: crc = ((crc >> 8) ^ CRC16_TAB[(crc ^ byte) & 0xFF]) & 0xFFFF
    return crc

def bits_to_bytes(bit_string):
    try:
        return bytes(int(bit_string[i:i+8], 2) for i in range(0, len(bit_string), 8))
    except:
        return b""

class JammingControllerNode(Node):
    def __init__(self):
        super().__init__('jamming_controller_node')
        self.publisher_ = self.create_publisher(String, 'radar/jamming_key', 10)
        self.enemy_camp = 'BLUE'
        self.get_logger().info('现场强化侦查模式已启动')

    def publish_key(self, key_str):
        msg = String()
        msg.data = key_str
        self.publisher_.publish(msg)
        self.get_logger().info(f'【!!!破获密钥!!!】: {key_str}')

def main():
    rclpy.init()
    node = JammingControllerNode()

    try:
        grc_rpc = xmlrpc.client.ServerProxy(RPC_URL)
    except Exception as e:
        print(f"RPC失败: {e}")
        return

    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.setblocking(False)
    sock.bind((UDP_IP, UDP_PORT))

    bit_buffer = ""
    serial_buffer = bytearray()
    
    last_success_time = time.time()
    last_scan_time = time.time()
    locked = False
    scan_idx = 0

    print("正在监听干扰信号...")

    while rclpy.ok():
        current_time = time.time()

        # 1. 自动跳频/巡逻逻辑
        if not locked and (current_time - last_scan_time > 2.0):
            config_list = FREQ_MAP[node.enemy_camp]
            scan_idx = (scan_idx + 1) % len(config_list)
            target_f, target_s = config_list[scan_idx]
            try:
                grc_rpc.set_target_freq(target_f)
                grc_rpc.set_target_sens(target_s)
                print(f"[*] 巡逻中 -> {target_f/1e6} MHz")
            except: pass
            last_scan_time = current_time
            bit_buffer = ""; serial_buffer.clear()

        # 2. 接收 UDP 原始比特流
        try:
            data, _ = sock.recvfrom(32768)
            bit_buffer += ''.join(str(b) for b in data)
            if len(bit_buffer) > 50000: bit_buffer = bit_buffer[-10000:] # 防止内存溢出
        except BlockingIOError: pass

        # 3. 第一层：【模糊同步引擎】
        while len(bit_buffer) >= AIR_FRAME_LEN * 8:
            # 搜索后缀 48 位，容忍开头的 bit-slip
            idx = bit_buffer.find(FUZZY_SYNC_BITS)
            if idx == -1:
                bit_buffer = bit_buffer[-63:] # 保留末尾防截断
                break
            
            # 计算这一帧在 buffer 里的真正起点 (往回退 16 位)
            frame_start_idx = idx - 16
            if frame_start_idx < 0:
                # 起点在缓冲区外面，丢掉这半截包
                bit_buffer = bit_buffer[idx:]
                continue

            # 抓取完整的 27 字节 (216 bit)
            frame_bits = bit_buffer[frame_start_idx : frame_start_idx + 216]
            frame_bytes = bits_to_bytes(frame_bits)
            
            # 即使 Header 对不上也塞进 serial_buffer，交给 CRC 最终裁决
            if len(frame_bytes) == 27:
                serial_buffer.extend(frame_bytes[12:27]) # 提取 15字节 Payload
                
            bit_buffer = bit_buffer[idx + 48:] # 移动到匹配点之后
        
        # 4. 第二层：【串口流自愈引擎】
        while len(serial_buffer) >= 9:
            sof_idx = serial_buffer.find(0xA5)
            if sof_idx == -1: serial_buffer.clear(); break
            if sof_idx > 0: serial_buffer = serial_buffer[sof_idx:]
            
            if len(serial_buffer) < 5: break
            # 帧头 CRC8 验证
            if calc_crc8(serial_buffer[:4]) != serial_buffer[4]:
                serial_buffer = serial_buffer[1:]; continue
            
            data_len = struct.unpack('<H', serial_buffer[1:3])[0]
            expected_len = 5 + 2 + data_len + 2
            
            if expected_len > 128: 
                serial_buffer = serial_buffer[1:]; continue
            if len(serial_buffer) < expected_len: break
            
            full_frame = serial_buffer[:expected_len]
            # 整包 CRC16 验证
            if calc_crc16(full_frame[:-2]) == struct.unpack('<H', full_frame[-2:])[0]:
                last_success_time = time.time()
                locked = True
                
                cmd_id = struct.unpack('<H', full_frame[5:7])[0]
                if cmd_id == 0x0A06:
                    raw_key = full_frame[7 : 7 + data_len]
                    # 只保留可见 ASCII 字符
                    clean_key = "".join([chr(b) for b in raw_key if 32 <= b <= 126]).strip()
                    if clean_key: 
                        node.publish_key(clean_key)
                        print(f"🔥 [SUCCESS] 捕获密钥: {clean_key} (Len:{data_len})")
            
            serial_buffer = serial_buffer[expected_len:]

        # 信号丢失保护
        if locked and (time.time() - last_success_time > 4.0):
            locked = False
            print("[!] 信号丢失超过 4s，重新搜寻...")

        rclpy.spin_once(node, timeout_sec=0.001)

if __name__ == '__main__':
    main()