...
# -*- coding: utf-8 -*-
"""
rm_jamming_mock_v2.py
=====================
模拟官方「干扰波」发射端（协议 V2.0.0 / 规则 V2.1.0），
直接喂当前 jamming_decoder_f2（无需 Pluto / GRC）。

---------------------------------------------------------------------------
与旧 rm_jamming_mock.py 的区别
---------------------------------------------------------------------------
  旧版：往 12347 发空口整包字节（供已删除的 GRC TX 通路）。
  本版：往 14348 发「每字节一个 bit（数值 0/1）」，对齐 GFSK Demod 输出。
...
UDP_IP = "127.0.0.1"
UDP_PORT = 14348          # = jamming_decoder_f2.UDP_PORT
ROUND_HZ = 10.0           # 官方密钥约 10 Hz
FILLER_BYTES = 120        # 与开源仓库 jamming round 一致：15+120=135
INVERT_POLARITY = False
BITS_PER_UDP = 512
KEY_ROTATE_ROUNDS = 50    # 每 N 轮换一把密钥（模拟战术改密）
...
def build_jamming_round(key: str, seq: int, fill_seed: int) -> bytes:
    """
    官方干扰一轮：0x0A06 整帧恰好 15 字节 + 随机填充。
    填充是裸字节，不是带 CRC 的假裁判帧。
    """
    ...
    assert len(frame) == 15, f"0x0A06 frame must be 15B, got {len(frame)}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="干扰波 V2 mock → jamming_decoder_f2")
    ...
    parser.add_argument("--key", default=None, help="固定密钥（默认轮换 KEYS 列表）")
    ...
    print(
        f"干扰波 mock V2 → {args.ip}:{args.port} | "
        f"{args.hz:.1f} Hz | filler={FILLER_BYTES}B | 反相={'开' if args.invert else '关'}"
    )
    print("请先启动 jamming_decoder_f2（建议 ENABLE_GRC_WATCHDOG=False）\n")
