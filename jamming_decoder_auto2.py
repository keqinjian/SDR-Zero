...
FREQ_MAP = {
    'RED':  [(432200000, 2.8323), (432500000, 2.5809), (432800000, 0.6646)],
    'BLUE': [(434320000, 0.6646), (434320000, 0.6646), (434320000, 0.6646)]
}
...
class JammingControllerNode(Node):
    def __init__(self):
        ...
        self.get_logger().info('现场调试模式已启动')

    def publish_key(self, key_str):
        msg = String()
        msg.data = key_str
        self.publisher_.publish(msg)
        self.get_logger().info(f'【现场拦截】成功获取密钥: {key_str}')
...
def main():
    ...
    print("[DEBUG] 进入原始比特监控模式...")

    while rclpy.ok():
        current_time = time.time()

        # 现场调试打印：每 1 秒打印当前接收到的比特流片段
        if current_time - last_debug_time > 1.0:
            if len(bit_buffer) > 128:
                snippet = bit_buffer[:128]
                # 打印前 128 bit 的 Hex 形式方便对比
                try:
                    hex_snippet = hex(int(snippet, 2))[2:].upper()
                    print(f"\n--- 现场原始比特流预览 (128bit) ---")
                    print(f"BIN: {snippet[:64]}...")
                    print(f"HEX: {hex_snippet}")
                    print(f"当前搜寻 AccessCode(MSB): {ACCESS_CODE_BITS[:16]}...")
                    # 检查备选序列是否出现在这一段里
                    if ACCESS_CODE_BITS_INV in bit_buffer: print(">> 检测到【极性翻转】特征！请修改解调器极性。")
                    if ACCESS_CODE_BITS_LSB in bit_buffer: print(">> 检测到【LSB序】特征！")
                except: pass
        ...
        try:
            ...
            # data 包含的是 0x00 和 0x01
            bit_buffer += ''.join(str(b) for b in data)
        except BlockingIOError: pass

        # 第一层解析：尝试 MSB 匹配
        while len(bit_buffer) >= AIR_FRAME_LEN * 8:
            # 现场调试：同时尝试 MSB, INV 和 LSB 匹配
            found = False
            for tag, code in [("MSB", ACCESS_CODE_BITS), ("INV", ACCESS_CODE_BITS_INV), ("LSB", ACCESS_CODE_BITS_LSB)]:
                idx = bit_buffer.find(code)
                if idx != -1:
                    if tag != "MSB": print(f"警告：发现 {tag} 匹配，当前程序正在使用 MSB 解析可能出错")
                    # 以找到的码为准切下数据
