...
RECORD_PREFIX = "info_record"       # 录制文件前缀
...
class InfoDecoderNode(Node):
    def __init__(self):
        ...
        self.get_logger().info('✅ 战术情报分发中心已就绪')
...
def parse_0x0A04(data, node):
    ...
    macro_data = {
        "coin": coin, "total_coin": total_coin, 
        "occupation": {
            "center_highland": (status >> 1) & 0x03,
            "base_shield": (status >> 8) & 0x01
        }
    }
...
def main():
    ...
    if RECORD_STREAM:
        try:
            ...
            ros_node.get_logger().info(f"💽 情报录制已开启: {current_record_file}")
    ...
    try:
        while rclpy.ok():
            ...
            while len(bit_buffer) >= AIR_FRAME_LEN * 8:
                ...
                start_idx = idx - 16
                if start_idx < 0:
                    bit_buffer = bit_buffer[idx:]; continue
