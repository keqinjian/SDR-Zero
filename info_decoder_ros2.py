...
# 兼容大疆的大小端乌龙
EXPECTED_HEADER_BE = bytes([0x00, 0x0F, 0x00, 0x0F]) # 实测大概率是这个
...
class InfoDecoderNode(Node):
    def __init__(self):
        ...
        # 创建 5 个独立的的话题，全部采用 JSON String 格式
        ...
        self.get_logger().info('ROS2)话题已就绪')
...
def parse_0x0A01(data, node):
    """0x0A01 对方机器人位置坐标 (24字节)"""
    if len(data) < 24: return
    
    # 🚨 终极修正：官方结构体为 uint16_t，必须用无符号的 <12H
    # 赛场坐标系全在第一象限，绝无负数
...
def parse_0x0A02(data, node):
    ...
    hp_data = {
        "hero": hero, "eng": eng, "inf3": inf3, 
        "inf4": inf4, "sentry": sentry
    }
    node.publish_data(node.pub_hp, hp_data)
...
def parse_0x0A03(data, node):
    ...
    ammo_data = {
        "hero": hero, "inf3": inf3, "inf4": inf4, 
        "aerial": aerial, "sentry": sentry
    }
    node.publish_data(node.pub_ammo, ammo_data)
...
def parse_0x0A04(data, node):
    ...
    # H=剩余金币, H=累计金币, I=32位状态位图
    coin, total_coin, status = struct.unpack('<H H I', data[:8])
    
    # 深度解构 32 位状态位图 (参考 PDF Page 40-41)
    macro_data = {
        "coin": coin, 
        "total_coin": total_coin, 
        "occupation": {
            "supply_zone":      (status >> 0) & 0x01,        # 补给区占领
            "center_highland":  (status >> 1) & 0x03,        # 中央高地 (0:未占, 1:对方占, 2:己方占)
            "trapezoid":        (status >> 3) & 0x01,        # 梯形高地
            "fortress":         (status >> 4) & 0x03,        # 堡垒增益点
            "outpost":          (status >> 6) & 0x03,        # 前哨站增益点
            "base_shield":      (status >> 8) & 0x01,        # 基地增益点 (护盾)
            "interaction_card": (status >> 9) & 0x7F         # 其他场地交互模块刷卡状态
        },
        "raw_status_bitmap": status
    }
...
def parse_0x0A05(data, node):
    ...
    for i in range(5):
        ...
        buff_data["units"][robot_keys[i]] = {
            "recovery": rec, "cooling": cool, 
            "defense": df, "vulnerability": vun, "attack": atk
        }
    ...
    try:
        while rclpy.ok():
            ...
            if current_time - last_print_time > 2.0:
                print(f"过去两秒接收包量: {packet_counter}")
                packet_counter = {k:0 for k in packet_counter} # 清零
            ...
            while len(serial_buffer) >= 9:
                ...
                if calc_crc16(full_frame[:-2]) == struct.unpack('<H', full_frame[-2:])[0]:
                    ...
                    # 路由解析与发布
                    if cmd_id == 0x0A01: 
                        parse_0x0A01(actual_data, ros_node)
                        packet_counter["0x0A01"] += 1
                    elif cmd_id == 0x0A02: 
                        parse_0x0A02(actual_data, ros_node)
                        packet_counter["0x0A02"] += 1
                    elif cmd_id == 0x0A03: 
                        parse_0x0A03(actual_data, ros_node)
                        packet_counter["0x0A03"] += 1
                    elif cmd_id == 0x0A04: 
                        parse_0x0A04(actual_data, ros_node)
                        packet_counter["0x0A04"] += 1
                    elif cmd_id == 0x0A05: 
                        parse_0x0A05(actual_data, ros_node)
