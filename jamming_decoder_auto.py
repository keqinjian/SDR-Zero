...
UDP_IP = "127.0.0.1"
UDP_PORT = 14348
RPC_URL = "http://127.0.0.1:8080" # GNU Radio XMLRPC 端口
...
class JammingControllerNode(Node):
    def __init__(self):
        ...
        self.enemy_camp = 'BLUE' # 默认蓝方
        self.get_logger().info('干扰波解析已启动')
...
def main():
    ...
    try:
        ...
        # 简单握手测试
        grc_rpc.set_target_freq(434920000) 
    except Exception as e:
        print(f"无法连接到 GNU Radio XMLRPC Server: {e}")
        return

    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.setblocking(False)
    sock.bind((UDP_IP, UDP_PORT))
    print(f"扫频已启动 (UDP {UDP_PORT}) ...")
        ...
        while rclpy.ok():
            ...
            if not locked and (current_time - last_success_time > 3.0):
                ...
                try:
                    ...
                    print(f"信号丢失，切换频点: {target_f/1e6} MHz (Sens: {target_s})")
                except:
                    pass
                
                # 重置时间，给新频率一点捕获机会
                last_success_time = current_time
                bit_buffer = "" # 清空缓冲区防止跨频率乱码
                serial_buffer.clear()
